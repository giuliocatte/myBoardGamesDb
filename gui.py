...
class Site:
	...
	@routing('/owned')
	def index(self):
		theme = bottle.request.params.get('theme', 'default')
		header = (
			('gameId', 'bgg id'),
			('name', 'Titolo'),
		...
		'''.format(
			...
			new=''.join('<tr>{}</tr>'.format(''.join(r)) for r in self.bodyrows(header, filter=(lambda x: x['yearPublished'] >= 1975))),
			classics=''.join('<tr>{}</tr>'.format(''.join(r)) for r in self.bodyrows(header, filter=(lambda x: x['yearPublished'] < 1975))),
			theme=theme,
